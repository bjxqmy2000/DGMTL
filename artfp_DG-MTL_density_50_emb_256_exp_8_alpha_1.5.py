...
class Dataset:
    """RTFP 数据集类，处理 RT 和 FP（失败概率）数据"""

    def __init__(self, userlist_path, wslist_path, data_dir):
        # 用户特征：user_id, country
        self.user_id_num = 0
        self.user_country_num = 0

        # 服务特征：ws_id, provider, country
        self.ws_id_num = 0
        ...
        # RT / FP 矩阵路径（训练集、验证集、测试集）
        rtMatrix_train_path = os.path.join(data_dir, "sparse/rtMatrix_train.csv")
        rtMatrix_val_path = os.path.join(data_dir, "sparse/rtMatrix_val.csv")
        rtMatrix_test_path = os.path.join(data_dir, "sparse/rtMatrix_test.csv")
        fpMatrix_train_path = os.path.join(data_dir, "sparse/fpMatrix_train.csv")
        fpMatrix_val_path = os.path.join(data_dir, "sparse/fpMatrix_val.csv")
        fpMatrix_test_path = os.path.join(data_dir, "sparse/fpMatrix_test.csv")
        ...
        fpMatrix_train_df = pd.read_csv(fpMatrix_train_path, header=None)
        fpMatrix_val_df = pd.read_csv(fpMatrix_val_path, header=None)
        fpMatrix_test_df = pd.read_csv(fpMatrix_test_path, header=None)
        ...
        self.train_fp_info = self.get_fp_info(fpMatrix_train_df)
        self.val_fp_info = self.get_fp_info(fpMatrix_val_df)
        self.test_fp_info = self.get_fp_info(fpMatrix_test_df)

        # 构建数据集
        self.build_dataset()

    def get_user_info(self, userlist_df):
        """获取并编码用户特征（user_id, country）"""
        userlist_df["user_id"] = userlist_df["user_id"].astype(int)

        lbe = LabelEncoder()
        userlist_df["country"] = lbe.fit_transform(userlist_df["country"])

        max_values = userlist_df[["user_id", "country"]].max()
    ...
    def get_ws_info(self, wslist_df):
        """获取并编码 Web 服务特征（ws_id, provider, country）"""
        wslist_df["ws_id"] = wslist_df["ws_id"].astype(int)

        for feat in ["provider", "country"]:
            lbe = LabelEncoder()
            wslist_df[feat] = lbe.fit_transform(wslist_df[feat])

        max_values = wslist_df[["ws_id", "provider", "country"]].max()
    ...
    def get_fp_info(self, fpMatrix_df):
        """处理失败概率矩阵"""
        fpMatrix_array = fpMatrix_df.values
        rows, cols = np.where(fpMatrix_array != -1)
        fps = fpMatrix_array[rows, cols]

        fp_info = {}
        for i, j, fp in zip(rows, cols, fps):
            if i not in fp_info:
                fp_info[i] = {}
            fp_info[i][j] = float(fp)

        logger.info("fp_info获取完毕，非零元素数量：{}".format(len(fps)))
        return fp_info
    ...
    def get_dataset(self, user_info, ws_info, rt_info, fp_info):
        """构建数据集：允许只含 RT 或 FP，但至少包含一个 QoS"""

        pairs = set()
        for user_id, services in rt_info.items():
            for ws_id in services.keys():
                pairs.add((user_id, ws_id))
        for user_id, services in fp_info.items():
            for ws_id in services.keys():
                pairs.add((user_id, ws_id))

        pairs = list(pairs)
        logger.info(f"有效样本对数量(至少包含RT或FP): {len(pairs)}")

        dataset = []
        for user_id, ws_id in pairs:
            has_rt = user_id in rt_info and ws_id in rt_info[user_id]
            has_fp = user_id in fp_info and ws_id in fp_info[user_id]

            if not has_rt and not has_fp:
                continue

            rt_value = rt_info[user_id][ws_id] if has_rt else -1.0
            fp_value = fp_info[user_id][ws_id] if has_fp else -1.0
...
class TorchDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {
            ...
            'fp': torch.tensor(float(item['fp']), dtype=torch.float32),
            'rt_mask': torch.tensor(1.0 if item.get('rt_present', True) else 0.0, dtype=torch.float32),
            'fp_mask': torch.tensor(1.0 if item.get('fp_present', True) else 0.0, dtype=torch.float32),
        }
...
def train_stem_model(model, train_loader, valid_loader, optimizer, scheduler, device,
    ...
    if model.use_gradnorm:
        ...
        weight_optimizer = optim.Adam([loss_weighting.task_weights], lr=gradnorm_lr)
    ...
    for epoch in range(num_epochs):
        ...
        for batch in train_loader:
            ...
            fp_targets = batch['fp'].to(device)
            rt_mask = batch.get('rt_mask', torch.ones_like(rt_targets)).to(device)
            fp_mask = batch.get('fp_mask', torch.ones_like(fp_targets)).to(device)
            ...
            fp_pred = preds[1].squeeze(-1)

            # 计算单独的损失
            rt_per = criterion(rt_pred, rt_targets)
            fp_per = criterion(fp_pred, fp_targets)

            rt_denom = torch.clamp(rt_mask.sum(), min=1.0)
            fp_denom = torch.clamp(fp_mask.sum(), min=1.0)
            rt_loss = (rt_per * rt_mask).sum() / rt_denom
            fp_loss = (fp_per * fp_mask).sum() / fp_denom
            ...
            if model.use_gradnorm:
                ...
                weighted_losses = loss_weighting([rt_loss, fp_loss])

                # 3. 使用Autograd计算GradNorm的损失 (此时不影响网络主干更新)
                loss_gradnorm = loss_weighting.compute_gradnorm_loss(
                    weighted_losses, [rt_loss, fp_loss], last_shared_params
                )
                ...
                pure_total_loss = detached_weights[0] * rt_loss + detached_weights[1] * fp_loss
                ...
                total_loss = rt_loss + fp_loss
                total_loss.backward()
                optimizer.step()

            train_loss += total_loss.item()
            rt_losses += rt_loss.item()
            fp_losses += fp_loss.item()
            batch_count += 1
        ...
        avg_fp_loss = fp_losses / batch_count if batch_count > 0 else 0
        ...
        if model.use_gradnorm:
            ...
            loss_ratios = loss_weighting.get_loss_ratios(
                [torch.tensor(avg_rt_loss, device=device), torch.tensor(avg_fp_loss, device=device)])
            logger.info(
                f"Epoch [{epoch + 1}/{num_epochs}], "
                f"训练损失: {avg_train_loss:.4f}, "
                f"RT损失: {avg_rt_loss:.4f}, FP损失: {avg_fp_loss:.4f}, "
                f"GradNorm权重: RT={weights[0].item():.4f}, FP={weights[1].item():.4f}, "
                f"损失比率: RT={loss_ratios[0].item():.4f}, FP={loss_ratios[1].item():.4f}, "
        ...
        if (epoch + 1) % eval_epochs == 0:
            ...
            logger.info(
                ...
                f"FP MAE: {valid_metrics['fp_mae']:.4f}, FP RMSE: {valid_metrics['fp_rmse']:.4f}, "
    ...
    logger.info(
        ...
        f"FP MAE: {best_metrics['fp_mae']:.4f}, FP RMSE: {best_metrics['fp_rmse']:.4f}, "
    )
...
def evaluate_stem_model(model, valid_loader, device):
    ...
    with torch.no_grad():
        for batch in valid_loader:
            ...
            if fp_mask.sum().item() > 0:
                fp_mask_bool = fp_mask > 0
                fp_preds.append(fp_pred[fp_mask_bool])
                fp_targets_list.append(fp_targets[fp_mask_bool])

    # 合并所有批次的预测和目标
    rt_preds = torch.cat(rt_preds) if len(rt_preds) > 0 else torch.tensor([], device=device)
    fp_preds = torch.cat(fp_preds) if len(fp_preds) > 0 else torch.tensor([], device=device)
    rt_targets = torch.cat(rt_targets_list) if len(rt_targets_list) > 0 else torch.tensor([], device=device)
    fp_targets = torch.cat(fp_targets_list) if len(fp_targets_list) > 0 else torch.tensor([], device=device)
    ...
    if fp_preds.numel() > 0:
        fp_mae = torch.mean(torch.abs(fp_preds - fp_targets)).item()
        fp_mse = torch.mean((fp_preds - fp_targets) ** 2).item()
        fp_rmse = torch.sqrt(torch.tensor(fp_mse)).item()
    else:
        fp_mae, fp_rmse = float('nan'), float('nan')
...
def main():
    """主函数：DEGN 模型用于 RT + FP（失败概率）多任务 QoS 预测"""
    parser = argparse.ArgumentParser(description='DEGN 模型用于 RTFP（RT + FP）QoS 预测')

    # 数据参数（RTFP 数据集，与 analysis_preference_rtfp 一致）
    parser.add_argument('--data_dir', type=str, default='data/RTFP/')
    parser.add_argument('--userlist_path', type=str, default='data/RTFP/userlist.csv')
    parser.add_argument('--wslist_path', type=str, default='data/RTFP/wslist.csv')
    ...
    parser.add_argument('--num_shared_experts', type=int, default=8)
    ...
    # 特征信息字典（与 analysis_preference_rtfp 一致）
    user_feature_info = {
    ...
    logger.info(
        ...
        f"FP MAE: {test_metrics['fp_mae']:.4f}, FP RMSE: {test_metrics['fp_rmse']:.4f}"
    )
